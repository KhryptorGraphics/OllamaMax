...
func TestAPIInferenceEndpoints(t *testing.T) {
	...
	if token == "" {
		t.Skip("Could not get authentication token, skipping inference tests")
	}
...
// TestAPIErrorHandling tests API error handling - DISABLED: duplicate of api_test.go
func TestAPIErrorHandling_Comprehensive(t *testing.T) {
	gin.SetMode(gin.TestMode)
	server := createTestAPIServer(t)
...
// createTestAPIServer is defined in test_helpers.go to avoid duplication

// BenchmarkAPIEndpoints benchmarks API endpoint performance - DISABLED: duplicate of api_test.go
func BenchmarkAPIEndpoints_Comprehensive(b *testing.B) {
	gin.SetMode(gin.TestMode)
